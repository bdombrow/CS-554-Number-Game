...
public class NumberPuzzle {
	...
	public int ScrambleNumberPuzzle() {
		...
		int BlankIndex = (nYNumberBlankBrick * nYNumberBrickCount)
				+ nXNumberBlankBrick;
		...
		for (int i = 0; i < maxMoves; i++) {
			...
			// blank space left
			case 1:
				if (nXNumberBlankBrick != 0) // cannot move left from leftmost
												// column
			...
			// blank space right
			case 2:
				if (nXNumberBlankBrick != nXNumberBrickCount - 1) // cannot move
																	// right
																	// from
					// rightmost column
					newIndex = BlankIndex + 1;
			...
			// blank space up
			case 3:
				if (nYNumberBlankBrick != 0) // cannot move up from the topmost
												// row
			...
			// blank space down
			case 4:
				if (nYNumberBlankBrick != nYNumberBrickCount - 1) // cannot move
																	// down from
					// the bottom most row
					newIndex = BlankIndex + nXNumberBrickCount;
				else
			...
			if (randomMoves[i] != 5) {
				...
				int x = newIndex % nXNumberBrickCount; // calculate x or column
														// index
				int y = (newIndex - x) / nYNumberBrickCount; // calculate y or
																// row
				// index
				...
				if (numRandomPuzzleMoves == numRandomMoves)
				{
					if ( nXNumberBrickCount == 2 )
					{
						...
						for (int k=0; k<adjust; k++)
						{
							...
							PlayBackMoves(randomPuzzleMoves,
									numRandomPuzzleMoves, singlestep);
							numRandomPuzzleMoves--;
							numPlayerPuzzleMoves--;
						}
						if (IsPuzzleSolved()) {
		...
		for (int y = 0; y < nYNumberBrickCount; y++) {
			for (int x = 0; x < nXNumberBrickCount; x++) {
				if (nNumberPuzzleGrid[x][y] != counter) {
					if (nNumberPuzzleGrid[x][y] == BLANK
							&& (x == (nXNumberBrickCount - 1))
		...
		for (i = 0; i < nXNumberBrickCount; i = i + 1) {
			for (j = 0; j < nYNumberBrickCount; j = j + 1) {
				map.putInt(Integer.toString(i) + "-" + Integer.toString(j),
						Integer.valueOf(nNumberPuzzleGrid[i][j]));
			}
		}
	...
	public void restoreState(Bundle icicle) {
		...
		for (i = 0; i < nXNumberBrickCount; ++i) {
			for (j = 0; j < nYNumberBrickCount; ++j) {
				nNumberPuzzleGrid[i][j] = icicle.getInt(Integer.toString(i)
						+ "-" + Integer.toString(j));
		...
		if (neadNewPuzzle) {
			nNumberPuzzleGrid = CreateNewNumberPuzzle(nXNumberBrickCount,
					nYNumberBrickCount);
		}
	}
	...
	public void saveState(SharedPreferences.Editor editor) {
		...
		for (i = 0; i < nXNumberBrickCount; i = i + 1) {
			for (j = 0; j < nYNumberBrickCount; j = j + 1) {
				editor.putInt("nNumberPuzzleGrid-" + Integer.toString(i) + "-"
						+ Integer.toString(j), nNumberPuzzleGrid[i][j]);
			}
		}
		editor.commit();
	}
	...
	public void restoreState(SharedPreferences settings) {
		...
		mNumberOptionChanged = settings
				.getBoolean("mNumberOptionChanged", true);
		nLevel = settings.getInt("mLevel2", 1);

		nNumberPuzzleGrid = new int[nXNumberBrickCount][nYNumberBrickCount];
		for (i = 0; i < nXNumberBrickCount; i = i + 1) {
			for (j = 0; j < nYNumberBrickCount; j = j + 1) {
				nNumberPuzzleGrid[i][j] = settings
						.getInt("nNumberPuzzleGrid-" + Integer.toString(i)
